...
class LevelThree(tk.Frame):
         ...
         try:
             if self.card_count==0:
                   if self.key1_button and self.key2_button !=None:
                       ...
                       self.text.set(str(self.count)+' / 15')
                       if self.count == 15 :
                           self.success()
                           print("종료")
                   return
    ...
    def gui_frame(self):
        ...
        fontStyle2 = tkFont.Font(family="G마켓 산스 Light", size=16, weight='bold')
        self.text = tk.StringVar()
        self.text.set('0 / 15')
         
        tk.Label(self, text='제한 시간(3분)', font=fontStyle2, bg='white', fg='black').place(x=140, y=160)
        self.score=tk.Label(self, textvariable=self.text, font=fontStyle, bg='white', fg='black').place(x=710, y=140)
